...
const UnstakeCard = ({ 
  ...
  const unstakeAmount = parseFloat(transaction?.amount || '0').toFixed(2);
  const estimatedReceive = parseFloat(transaction?.estimatedReceive || '0').toFixed(2);
  const processingTime = transaction?.processingTime || '48-72 hours';
  ...
  const confirmTransaction = async (transaction) => {
    try {
      ...
      console.log("Starting unstake transaction", transaction);
      ...
      // Perform unstake operation directly
      const resp = await tonstakers.unstake(toNano(transaction.amount));
      
      console.log("Unstake result:", resp);
      ...
      console.log("Unstake transaction error:", error);
      ...
      if (error.message && error.message.includes('User rejects the action')) {
        ...
        errorMsg = error.message || 'Unstake transaction failed. Please try again.';
      }
    ...
    <div className="flex flex-col items-center w-full max-w-[267px] mx-auto">
      ...
      <div className="relative w-[265px] h-[205px] bg-[#1a1c1e] rounded-3xl border border-[#1a1c1e] shadow-[3px_4px_4px_-2px_rgba(211,255,202,0.25)]">
        <div className="flex flex-col h-[205px] items-center justify-start overflow-clip pb-0 pt-4 px-0 relative w-[265px]">
          ...
          {/* Header with Unstake title and balance */}
          <div className="flex items-center justify-between px-6 py-0 relative shrink-0 w-full">
            <div className="flex gap-2.5 items-center justify-center overflow-clip pb-4 pt-0 px-0 relative shrink-0">
              <span className="font-['Inter:Regular',_sans-serif] font-normal text-[#707173] text-[14px] leading-[20px]">Unstake</span>
            </div>
            <div className="flex gap-2.5 items-center justify-center overflow-clip pb-4 pt-0 px-0 relative shrink-0">
              ...
              <div className="flex items-end justify-end pl-0 pr-3 py-0 relative shrink-0">
                <div className="mr-[-12px] relative rounded-[100px] shrink-0 size-9">
                  <div className="w-9 h-9 bg-[#00D4AA] rounded-full flex items-center justify-center">
                    <span className="text-white font-bold text-[12px] leading-[16px]">tsTON</span>
                  </div>
                </div>
              </div>
              <div className="flex flex-col gap-1 items-start justify-start leading-[0] not-italic relative shrink-0">
                <span className="font-['Inter:Medium',_sans-serif] font-medium text-[#ffffff] text-[16px] leading-[24px]">tsTON</span>
                <span className="font-['Inter:Regular',_sans-serif] font-normal text-[#707173] text-[12px] leading-[16px]">Unstake Amount</span>
              </div>
            </div>
            <div className="flex flex-col gap-1 items-end justify-center leading-[0] not-italic relative shrink-0 text-right">
              <span className="font-['Inter:Medium',_sans-serif] font-medium text-[#ffffff] text-[16px] leading-[24px]">{unstakeAmount}</span>
              <span className="font-['Inter:Regular',_sans-serif] font-normal text-[#707173] text-[12px] leading-[16px]">{processingTime}</span>
            </div>
          </div>
          ...
          <div className="flex h-[70px] items-center justify-between overflow-clip p-6 relative shrink-0 w-full">
            <div className="flex gap-3.5 items-center justify-start relative shrink-0">
              ...
              <div className="flex flex-col gap-1 items-start justify-start leading-[0] not-italic relative shrink-0">
                <span className="font-['Inter:Medium',_sans-serif] font-medium text-[#ffffff] text-[16px] leading-[24px]">Receive</span>
                <span className="font-['Inter:Regular',_sans-serif] font-normal text-[#707173] text-[12px] leading-[16px]">Estimated Amount</span>
              </div>
            </div>
            <div className="flex flex-col gap-1 items-end justify-center leading-[0] not-italic relative shrink-0 text-right">
              <span className="font-['Inter:Medium',_sans-serif] font-medium text-[#ffffff] text-[16px] leading-[24px]">{estimatedReceive}</span>
      ...
      {transactionState === 'success' && (
        <div className="mt-4 p-2 bg-green-900/20 border border-green-600 rounded-lg w-full">
          <p className="text-green-400 text-[10px] leading-[14px]">Unstake transaction completed successfully!</p>
        </div>
      )}
    </div>
  );
